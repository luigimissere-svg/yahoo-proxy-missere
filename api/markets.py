...
# market_expectation: variazione bps attesa dal mercato (negativo = taglio, positivo = aumento, 0 = invariato)
# probability: probabilità attesa dai mercati (CME FedWatch, OIS curves)
CENTRAL_BANKS = {
    'ECB': {
        'name': 'BCE (Eurozona)',
        'rate': 2.25,
        'next_meeting': '2026-06-04',
        'market_expectation_bps': -25,
        'probability_pct': 68,
        'expectation_label': 'Taglio -25bp',
    },
    'FED': {
    ...
    'BOE': {
        ...
        'market_expectation_bps': 0,
        'probability_pct': 72,
        'expectation_label': 'Invariato',
    },
    'BOJ': {
        'name': 'Bank of Japan',
        'rate': 0.50,
        ...
        'market_expectation_bps': 25,
        'probability_pct': 45,
        'expectation_label': 'Aumento +25bp',
    },
}
...
def build_markets():
    ...
    for k, v in CENTRAL_BANKS.items():
        try:
            meeting = datetime.strptime(v['next_meeting'], '%Y-%m-%d').date()
            days = (meeting - today).days
        except Exception:
            days = None
        cb_with_countdown[k] = {**v, 'days_to_meeting': days}
    result['central_banks'] = cb_with_countdown
    return result
