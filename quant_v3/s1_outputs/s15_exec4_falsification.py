"""
S1.5 esecuzione 4 — Test di falsificazione H1-H4 (cap 20 / per-ticker 0.05)

Script eseguito al termine del run autoritativo wf_runner --grid s1_5_exec3
--max-positions 20 --per-ticker-cap 0.05 (esec 4).

Ipotesi pre-registrate (preregistration_s15_exec4_cap_20_005.md SHA fa3b149a):
...
Output: s15_exec4_falsification_report.md + JSON dettagliato.

USO:
    python s1_outputs/s15_exec4_falsification.py
"""
...
RESULTS_CSV = BASE / "s15_exec4_f2_results.csv"
EQUITY_CSV = BASE / "s15_exec4_f2_equity.csv"
TRADES_CSV = BASE / "s15_exec4_f2_trades.csv"
STABILITY_JSON = BASE / "s15_exec4_f2_stability.json"
OUT_MD = BASE / "s15_exec4_falsification_report.md"
OUT_JSON = BASE / "s15_exec4_falsification_report.json"
...
def main() -> None:
    ...
    report = {
        ...
        "preregistration_sha": "fa3b149a86bf10ed6b7011e9fca11026934d0c68c3c147f2df385a99302bec61",
        "diagnostica_sha": "a82c8c1e2f633118db8a86214c0a3aa8c50160ddcbd3ec36b54c29ff0fe7d588",
    ...
    lines = [
        ...
        f"**Diagnostica BUY pre-cap SHA**: `{report['diagnostica_sha']}`",
        f"**Grid**: {report['grid']}",
        ...
        "",
        f"— Generato automaticamente da `s15_exec4_falsification.py`, {datetime.now(CEST).isoformat()}",
    ]
    OUT_MD.write_text("\n".join(lines))
